...
//Capturar el elemnto main
let divContenido = document.getElementById("contenido");


listaPlatillos.forEach(function(platos){
   let  divimag=document.createElement("div");
   divimag.setAttribute("class","imagen");
   divimag.innerHTML=`<img src=${platos.imagen}></img>`;
   divContenido.appendChild(divimag);

   let divtexto=document.createElement("div");
   divtexto.innerHTML=`<h4>${platos.nombre}</h4>
                        <p>${platos.descripcion}</p>
                        `;
  
        let divprecio=document.createElement("div");
        divprecio.setAttribute("class", "precio");
        divprecio.innerHTML=`<span>S/. ${platos.precio}</span>`
